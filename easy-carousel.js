var carouselInterval, 
    ...
    <div id="carousel::#CAR_INDEX#" class="#CLASS#  max-h-screen carousels bg-center bg-no-repeat bg-cover bg-fill opacity-30 absolute top-0 bottom-0 left-0 right-0" 
         style="background-image: url(#IMG_PATH#) !important; background-color: darkgray; #OPACITY#" >
    </div>
`;

function Initial(hostElementId, list, classname = null, opacity = null) {
    carouselHostElement = document.getElementById(hostElementId.replace('#', ''));
    carouselHostElement.style.position = 'relative';
    let index = 0;
    carouselItemHtmlString = carouselItemHtmlString.replace('#CLASS#', classname ?? '').replace('#OPACITY#', (opacity ?? `opacity: ${opacity} !important`));
